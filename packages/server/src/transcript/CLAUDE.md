...
- `readRange`'s `totalBytes` parameter (passed by 6F from `sessions.total_bytes`) can lag the on-disk sidecar by up to 1 s on a live session per [ND-13](../../../../docs/open-questions.md#nd-13-byte-accounting-cadence-for-sessionstotal_bytes). The half-open `[max(0, before - limit), before)` math from ND-04 stays correct under this lag — the client walks backward from `totalBytes`, and the missing leading-edge bytes show up on the next page request after the byte-accountant's next flush. `total_bytes === fstat(sidecar).size` is guaranteed only after `pty.onExit` drain (per-session) or `registry.shutdown()` drain (global). Callers needing exact byte-for-byte agreement should `fstat` the sidecar; the pagination contract does not.
