...
// run `pnpm --filter @techgardencode/relay fix-pty`. On Linux/Windows there is no
...
export function spawnHelperReady(): { ok: boolean; reason: string } {
  ...
  for (const rel of matches) {
    ...
    if (mode === 0) {
      return {
        ok: false,
        reason: `spawn-helper missing exec bit at ${rel} — run \`pnpm --filter @techgardencode/relay fix-pty\``,
      };
    }
  }
