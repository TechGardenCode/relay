...
- **Project-marker mental-model gap (corrected by [[nd-33-ide-gui-overhaul-deep-dive]]).** This bullet originally claimed `relay project add` does *not* write the `.relay/project.json` marker. That framing was stale — it predated the 6F `POST /projects` implementation. In current code, `relay project add` calls the same `POST /projects` handler the extension uses ([[d-12-project-record-storage-and-relay-project-add-semantics]] rules 6–7), which writes the marker and appends to `.gitignore`. The CLI-first operator's project *is* immediately bindable in the IDE in the same-host and Remote-SSH topologies. The only genuine residual is the true cross-device case (server filesystem ≠ IDE filesystem), where the server-written marker is not on the IDE's disk; ND-33 documents this as a known friction point and routes its diagnostic to [[nd-35-diagnostics-and-error-ux-deep-dive]]. Cites [[d-12-project-record-storage-and-relay-project-add-semantics]], [[d-g6-project-discovery-workspace-to-project-binding]], [[nd-07-marker-file-schema]].
...
*Resolved by [ND-35](../decisions/ND-35-diagnostics-and-error-ux-deep-dive.md) on 2026-05-27 — the painless-rollout bar for this surface lives in that resolution (quoted into §7 by link, per D-15 ownership). P0 shipped: `relay doctor`, a read-only probe set (credentials, `claude`-on-PATH, persona parse, storage, server reachability) that turns the silent-failure class into actionable remediation text; REST recovery paths documented per `type` code in [`rest-conventions.md`](rest-conventions.md) §7. Deferred with rationale: the pre-spawn credential gate and the machine-readable `recovery` field (P1, no consumer / partial-gate cost), in-IDE log access (P1, security-surface fan-out), attach-exit notifications (items e/f, gated on [[nd-30-relay-attach-stderr-event-stream-for-subprocess-of-attach-consumers]]), the marker-mismatch upgrade notice (gated on the extension surface + [[nd-31-secretstorage-key-namespace-for-multi-server-ide-pairing]]), telemetry (out of posture), and a `relay audit` surface (Phase 3). The D-G1 non-fatal persona-load and D-10 env-pass-through credential contracts stand unchanged.*
