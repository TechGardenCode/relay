...
`resize` is a **side-channel**: it is independent of the §5.1 claim-lock FSM, accepted from any attached connection regardless of claim state, and triggers no server→client acknowledgement. The server records each attached connection's last-reported `(cols, rows)` and recomputes the PTY's *effective* size on every attach, detach, and `resize` frame, forwarding the result to `node-pty`'s `resize(cols, rows)` only when it actually changes (no redundant SIGWINCH churn on a no-op recompute); it emits no frame in response. Each reported dimension is capped at 1000 before the effective size is computed. Clients SHOULD emit one `resize` immediately after the WebSocket upgrade (before any `claim`) so the PTY's reported dimensions match the operator's viewport before any TUI agent draws its first frame, and SHOULD re-emit on SIGWINCH (or the host's equivalent terminal-resize signal). The matching `process.stdout.on('resize', …)` listener is the canonical Node implementation.

**Multi-client policy** ([ND-39](../decisions/ND-39-concurrent-multi-client-attach-tui-rendering-corruption.md), amending [ND-23](../decisions/ND-23-pty-size-negotiation-and-sigwinch-forwarding-for-attach-clients.md)):

- **Exactly 1 connection attached — last-writer-wins.** The effective size is that connection's last-reported size, matching `ssh` / `tmux attach` / `screen` reference behavior.
- **≥2 connections attached — clamp to smallest.** The effective size is `(min(cols), min(rows))` across all currently-attached connections. This guarantees no client receives cursor-positioning escapes written outside its viewport — the corruption a full-screen TUI agent (claude) otherwise produces when two differently-sized clients stay co-attached. The larger client sees blank margins (the documented residual; correct rendering, not a shredded frame). On the 2→1 transition the PTY resizes **up** to the remaining client's full size and the TUI redraws via the resulting SIGWINCH.

The clamp changes only PTY *dimensions* — every attached connection still receives the full PTY byte stream per the universal-output rule (§2.4 / [D-G2](../decisions/D-G2-multi-client-input-arbitration.md) §5.1 rule 5, unchanged). The initial PTY spawn defaults to 120×32 (per `pty/supervisor.ts`); every realistic flow attaches within a tick, so the initial `resize` frame lands before any first-frame TUI draw.

*Multi-client clamp resolved by [ND-39](../decisions/ND-39-concurrent-multi-client-attach-tui-rendering-corruption.md) on 2026-05-28, amending [ND-23](../decisions/ND-23-pty-size-negotiation-and-sigwinch-forwarding-for-attach-clients.md) (the single-attach last-writer-wins case is unchanged).*
...
The `Backoff → Claiming-if-queued` transition implements [ND-02](../decisions/ND-02-rejection-ux-for-busy-response.md) for streaming: the notice still auto-dismisses after 4 seconds, the buffered input is preserved unchanged, and the client never auto-retries on per-keystroke cadence — the 4 s window is the rate-limit. For line-mode IDE clients, `pendingInput` only ever holds a complete draft, so the same transition cleanly resumes a single deliberate retry. [[nd-17-relay-attach-raw-mode-tty-variant-of-the-51-client-fsm]] (resolved 2026-05-27) confirms the canonical reading: §5.1 is a **wire-correctness** contract that names the IDE compose-field as one client UX without forbidding compatible variants, and the raw-mode `Idle → Claiming → Streaming → Idle` path documented above *is* the sanctioned variant — no separate §5.1.1 addendum is needed. The collapsed reading still applies to non-TUI line-mode agents (`bash -i`, scripted runs) where `pendingInput` is always a full line; ND-24 supersedes ND-17 for TUI agents.
...
**Browser variant (per [ND-36](../decisions/ND-36-subprotocol-sourced-bearer-token-for-browser-ws-auth.md)).** Browsers cannot set `Authorization` on `new WebSocket(...)`, so a browser client authenticates the upgrade via subprotocol negotiation instead: `new WebSocket(url, ['relay.bearer', token])`, serialized on the wire as `Sec-WebSocket-Protocol: relay.bearer, <token>`. The server uses the second CSV part as the bearer plaintext only when the first part is exactly `relay.bearer` and no `Authorization` header is present — `Authorization` always wins when both are supplied, and verification routes through the same token store either way. The 101 response echoes only the scheme name (`Sec-WebSocket-Protocol: relay.bearer`); the token is never reflected. Native clients (`relay attach`, the IDE extension via `relay attach`) keep using `Authorization` and never negotiate a subprotocol.
...
| 8 | PTY size negotiation | Client → server `resize { cols, rows }` side-channel; last-writer-wins while exactly 1 client attached, clamp to `min(cols,rows)` while ≥2 attached | §2.2 — [ND-23](../decisions/ND-23-pty-size-negotiation-and-sigwinch-forwarding-for-attach-clients.md) matches `ssh`/`tmux attach` for the single-attach case; [ND-39](../decisions/ND-39-concurrent-multi-client-attach-tui-rendering-corruption.md) clamps the multi-attach case so a TUI agent renders legibly on both clients |
