...
| 6Z | Phase 1 done gate — `scenario-runner` walks A, C–G + 1B/1C land | — | pending → [`docs/phase-1-acceptance-walk.md`](phase-1-acceptance-walk.md) (2026-05-22: under A–H gate, blocked on H.1 + H.3; [[d-16-phase-1-ships-without-distribution]] re-scoped to A–G, then [[d-17-personas-descoped-from-mvp]] dropped scenario B + task 1A → gate is **A, C–G** with 1B/1C; walk converts to pass — re-walk or accept evidence as-is) |
...
**Surfaces:** [`7H`](#7h-nd-39-concurrent-multi-client-tui-rendering--pending) — ND-39 concurrent-attach TUI corruption (needs an accept/clamp/per-client policy decision first).

### 7H. ND-39 concurrent multi-client TUI rendering — **pending**

**Goal:** Resolve [`ND-39`](decisions/ND-39-concurrent-multi-client-attach-tui-rendering-corruption.md) (open) — concurrent multi-client attach to a TUI agent renders corrupted (ND-23 last-writer-wins viewport conflict). **Needs a policy decision first** (accept-and-document / clamp-to-smallest-while-multi-attached / per-client render) via `decision-log` before any code — explicitly out of scope for 7G. The `nd39-*` tui-visual guards stay `it.fails` until then.

**Sequences after:** 7G (shares the attach surface + tui-visual harness). **Gates:** wider rollout.
