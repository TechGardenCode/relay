...
| 6L | Per-keystroke input streaming for TUI agents ([ND-24](open-questions.md#nd-24-per-keystroke-input-streaming-for-tui-agents)) | 6I, scenarios E/F/H with claude TUI | **pending** (design in ND-24; awaiting sign-off before code lands) |
...
**Goal:** land Option A per [ND-24](open-questions.md#nd-24-per-keystroke-input-streaming-for-tui-agents): per-keystroke `send` frames with claim-held-during-typing and **server-side Enter-byte detection** as the release trigger. Today's line-buffered `relay attach` is wire-correct but breaks the production agent — claude's TUI compose box never shows in-progress typing because `tty.ts`'s `lineBuffer` swallows every keystroke until Enter (verified on the 2026-05-19 6H walk). This task unblocks scenarios E/F/H with claude as the agent and unblocks 6I (the IDE extension's terminal widget hits the same compose-box bug the moment it spawns `relay attach`).

**Preflight gate:** ND-24 must flip from `in-deliberation` to `resolved` before code lands. The implementation contract below is what ND-24's Current thinking proposes; if a human reviewer picks a different option during sign-off, this stub adjusts before pickup.

**Output (planned):**
- `packages/server/src/server/ws/handler.ts` — `handleSend` (lines 386-426) stops auto-releasing the claim on every PTY write. A new helper scans the decoded payload for `\n` or `\r`; only when one is present does the function call `state.lock.releaseAsHolder(ctx.id, 'delivered')`. The existing `send_without_claim` error path still fires when a non-holder sends.
- `packages/server/src/attach/tty.ts` — drop `lineBuffer`, `consumeBuffer`, and `submitLine`. Forward each non-`^D` stdin byte verbatim through a new `client.submitByte(byte)` method. `^C` becomes a regular byte; no special-casing.
- `packages/server/src/attach/client.ts` — FSM gains a `streaming` state between `claiming` and `idle`. `claim_ack` → `streaming` (replaces the current ND-17 "collapse to `sending`" path). In `streaming`, each `submitByte` emits a `send` frame; state does not transition. `claim_released { delivered }` → `idle` and kicks a new `claim` if `pendingByte` is queued. `pendingByte` replaces `pendingLine`.
- `packages/protocol/src/ws-frames.ts` — schema unchanged; the `SendFrame` JSDoc updates to reflect the multi-send contract.
- `docs/arch/ws-protocol.md` — §2.2 catalog text for `send` updates ("one-or-more `send` frames per claim; the server releases when it sees a newline byte in any `send`'s decoded payload"), §5.1 client-FSM diagram + transition table gain the `Streaming` state, §5.2 server-FSM row 4 trigger changes from "delivered to PTY" to "send contains newline byte", §5.3 gains a race row for "newline mid-stream while another claim queued", §8 summary table adds the per-keystroke entry.
- `.claude/skills/ws-protocol-check/SKILL.md` — catalog gains the newline-release rule so the checker can validate it on future handler diffs.
- Tests across `packages/server/src/attach/client.test.ts` + `packages/server/src/attach/tty.test.ts` + `packages/server/src/server/ws/handler.test.ts` — multi-send-per-claim happy path; newline triggers release; mid-stream `claim_released { timeout }` returns to idle without auto-resend; `pendingByte` continuation under paste with embedded newline; `^C` passes through as a regular byte.
- ND-24 flips from `in-deliberation` to `resolved (YYYY-MM-DD)` with `Propagated to:` listing every site above. [ND-17](open-questions.md#nd-17-relay-attach-raw-mode-tty-variant-of-the-51-client-fsm) gains a final pointer to ND-24 (it stays `open` if line-mode-agent semantics still need their own §5.1.1 footnote; or resolves alongside ND-24 if the footnote collapses into ND-24's spec edits).

**Done when:** the single-device repro from ND-24's `Surfaced by` block runs end-to-end with `@anthropic-ai/claude-code` as the spawned agent and shows live keystrokes inside claude's compose box during `relay attach`. Vitest suite stays green (target: the existing 371-test baseline plus the new FSM specs). `pnpm typecheck` + `pnpm lint` clean. `vm-e2e` walk passes scenarios E/F over LAN with claude on both ends.

**Reads:** [`docs/open-questions.md`](open-questions.md) → [ND-24](open-questions.md#nd-24-per-keystroke-input-streaming-for-tui-agents), [ND-17](open-questions.md#nd-17-relay-attach-raw-mode-tty-variant-of-the-51-client-fsm), [D-G2](open-questions.md#d-g2-multi-client-input-arbitration), [ND-01](open-questions.md#nd-01-claim-lock-timeout-duration); [`docs/arch/ws-protocol.md`](arch/ws-protocol.md) §2.2 + §5.1 + §5.2 + §5.3; [`packages/server/src/attach/tty.ts`](../packages/server/src/attach/tty.ts); [`packages/server/src/attach/client.ts`](../packages/server/src/attach/client.ts); [`packages/server/src/server/ws/handler.ts`](../packages/server/src/server/ws/handler.ts). Use the [`build-plan-kickoff` skill](../.claude/skills/build-plan-kickoff/SKILL.md) to expand this stub into the full five-step kickoff at task pickup time.
