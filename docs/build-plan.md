...
| 6J | Distribution: npm tarball, Docker image, Compose | scenario H | **npm slice implemented** on branch `worktree-feat+npm-distribution` ([`docs/superpowers/plans/2026-07-02-npm-distribution.md`](superpowers/plans/2026-07-02-npm-distribution.md), per [[d-19-npm-distribution-posture]]) — pending operator publish (`NPM_TOKEN` + `v0.1.0` tag). Docker image, Compose, Helm chart, and extension-marketplace listing remain **deferred → [Track 8](#track-8--post-20-deferrals)** _(per [[d-16-phase-1-ships-without-distribution]])_ |
...
| P2-PWA-build | Phase 2 PWA — build `packages/pwa/` (Angular 21) from the L0–L6 spec | Phase 2 PWA ships | **done** → [`packages/pwa/`](../packages/pwa/) · plan [`docs/superpowers/plans/2026-07-02-p2-pwa-build.md`](superpowers/plans/2026-07-02-p2-pwa-build.md). Angular 21 (zoneless/standalone/signals, Vitest), `@angular/pwa` (installability only — NFR-7), xterm.js viewport, `@relay/protocol` workspace dep, Tailwind v4 placeholder seam (foundations §4). **Shipped FRs:** 1 (pair — D-13/ND-36/[`ND-47`](decisions/ND-47-pwa-api-base-and-server-url-model.md) API-base), 2/3 (home — ND-43 client-derived-status stopgap), 4 (xterm live — ND-40 buffer + D-G3), 5 (control rail — ND-42 claim-aware send-then-release stopgap), 6/7 (compose + raw — ND-24), 8 (spawn), 11/14 (detach/release/kill, fire-and-forget), 12 (BUSY — ND-02); NFR-2 reconnect (L3 §3.2) + NFR-10 status surface. **Deferred (NFR-5, stubbed + cited):** FR-9/FR-10 → [`ND-41`](decisions/ND-41-scratch-create-by-path-project-registration.md) (scratch/create-project — server pass), FR-13 → [`ND-44`](decisions/ND-44-file-viewer-server-surface.md) (file viewer — fast-follow). Two decisions filed: ND-47 (resolved — API base from D-13 `url`), [`ND-48`](decisions/ND-48-control-rail-arrow-key-escape-sequences.md) (open — arrow DECCKM stopgap). 44 Vitest specs green; whole-repo typecheck/lint clean; browser smoke verified (guard bounce · pair render/reject · ND-47 probe-to-pair-link-URL). **Pending live verification:** tui-visual xterm pass · full naive-user flow · LAN/Tailscale run on a real phone (needs `relay-deploy` VM + device — user-triggered) |
...
### 6J. Distribution: npm tarball, Docker image, Compose — **npm slice implemented; Docker/Compose deferred to [Track 8](#track-8--post-20-deferrals)**

Per [[d-16-phase-1-ships-without-distribution]] (2026-05-22), 6J moved to Track 8 (post-2.0) in full. Per [[d-19-npm-distribution-posture]] (2026-07-02), the operator picked the npm tarball slice back up ahead of the rest of Track 8: `@techgardencode/relay` + `@techgardencode/protocol` publish in lockstep from a `v*` tag with npm provenance, `node-pty` bumped to a Linux-prebuilt beta ([[nd-45-node-pty-linux-prebuild-resolution]]) for zero-build-tools install on macOS/Linux glibc, a postinstall repair/verify script, and a `relay doctor` native-addon probe. Implemented on branch `worktree-feat+npm-distribution` ([`docs/superpowers/plans/2026-07-02-npm-distribution.md`](superpowers/plans/2026-07-02-npm-distribution.md)); publishing itself is operator-gated (`NPM_TOKEN` secret + pushed `v0.1.0` tag) and has not happened yet. The full Track 8 row body below still lives there for the parts that remain deferred — Docker image, Docker Compose (Caddy + Tailscale sidecar), the Helm chart, and the extension-marketplace listing. Track 7 ND-32..35 resolutions inform the remaining Track 8 scope when that work is picked up.
...
**Status (2026-07-02): npm slice implemented, pending operator publish.** Per [[d-19-npm-distribution-posture]], the npm tarball half of this row shipped on branch `worktree-feat+npm-distribution` — see [`docs/superpowers/plans/2026-07-02-npm-distribution.md`](superpowers/plans/2026-07-02-npm-distribution.md) for the task-by-task record: package rename to `@techgardencode/relay` + `@techgardencode/protocol`, publish metadata, `node-pty` bumped to a Linux-prebuilt beta ([[nd-45-node-pty-linux-prebuild-resolution]] — closes Phase 0 surprise §5 below), a postinstall repair/verify script, a `relay doctor` native-addon probe, `ci.yml` (validate on push/PR), and `release.yml` (publish both packages with provenance on a `v*` tag). No `tsup` bundle was needed — the existing `tsc -b` build output publishes directly. **Not done:** the actual npm publish (operator-gated: set `NPM_TOKEN`, push a `v0.1.0` tag), the Docker image, `docker-compose.example.yml`, the GitHub Releases `.vsix` attachment, and the Helm chart — those remain open Track 8 work below.

**Goal:** npm-publishable tarball; Docker image; Docker Compose example with Caddy + Tailscale sidecar; GitHub Releases vsix attachment.
**Output:** root `Dockerfile`, `docker-compose.example.yml`, ~~npm publish workflow~~ (done — [`.github/workflows/release.yml`](../.github/workflows/release.yml)), release workflow attaching the `.vsix`.
**Done when:** scenario H — `npm install -g` on Node 22+ produces a working `relay` (mechanism implemented; first publish outstanding); the Docker image runs scenarios A–E from a clean container; the `.vsix` installs into VS Code from a GitHub Release.
**Reads:** [`docs/prd/06-distribution.md`](prd/06-distribution.md), [[d-16-phase-1-ships-without-distribution]], [[d-19-npm-distribution-posture]], [[nd-32-install-and-onboarding-deep-dive]] (constrains scope when resolved).
**Phase 0 surprises owned:** §1 (macOS spawn-helper chmod under pnpm — closed by `packages/server/scripts/postinstall.mjs`) and §5 (Linux `build-essential` requirement / `node-pty` Linux prebuild — closed for glibc x64/arm64 by [[nd-45-node-pty-linux-prebuild-resolution]]) — see [`docs/phase-0-report.md`](history/phase-0-report.md). Both blocked H.1 / scenario E on a clean host without the fixup; both are now fixed pending the first publish.
