...
| 7C-polish | IDE picker filters + REST-poll status-bar enrichment — P1 | 7F | **done** → grouped/filterable persona + attach quick-picks (`matchOnDescription`/`matchOnDetail` + `QuickPickItemKind.Separator` grouping via `quickPickGroups.ts`), REST-poll running-session indicator in `statusBar.ts` reusing a shared `PollLoop` (`poll.ts`, extracted from `sessionsTree.ts`) per [`ND-37`](decisions/ND-37-rest-poll-cadence-and-lifecycle-for-extension-live-surfaces.md) #6; 23 new co-located specs (57 total) |
...
**Validation:** suite 514/514 (was 501), typecheck + lint clean; ledger with pasted RED/GREEN evidence appended to [`rollout-readiness-walk.md`](rollout-readiness-walk.md) §"7G follow-up". Outstanding: **#10 `vm-e2e`** (real-LAN proof) is operator-driven — VM SSH is auto-mode-classifier-blocked from this runner (same blocker as 7F scenario E); the local real-binary proof (#6, fresh dist, 10/10) stands in. Diff drift-audited via `relay-spec-reviewer` (D-G2/D-G3/ND-24/ND-25/ND-34 intact).
