...
| 3E | `persona-yaml-check` skill | 6C, 1A | **done**, **deferred → Phase 2 ([[d-17-personas-descoped-from-mvp]])** — skill stays installed but personas are off the MVP path → [`.claude/skills/persona-yaml-check/SKILL.md`](../.claude/skills/persona-yaml-check/SKILL.md) |
...
| 6Z | Phase 1 done gate — `scenario-runner` walks A, C–G + 1B/1C land | — | **done** → [`docs/phase-1-acceptance-walk.md`](phase-1-acceptance-walk.md) "2026-05-28 gate-close addendum" (accept-with-delta: A,C–G **PASS** on the verified evidence ledger — A/C/D/G ← 2026-05-22 walk, E ← 7G `vm-e2e` 2026-05-28, F ← 7H clamp + ND-39 — with the suite green at HEAD `13c2d2d`; B deferred → Phase 2 per [[d-17-personas-descoped-from-mvp]], H deferred → Track 8 per [[d-16-phase-1-ships-without-distribution]]) |
...
| P2-PWA-tech | Phase 2 PWA — tech/architecture phase (framework, voice engine, terminal-on-mobile, PWA infra) → then `writing-plans` → build | Phase 2 PWA ships | pending (needs P2-PWA-design) — **deferred** per [[d-18-pwa-terminal-substrate-and-mvp-scope]] §7; re-engagement prompt in the design plan §8. Resolves the [`server-touchpoints.md`](design/pwa/server-touchpoints.md) additive surface into concrete protocol/REST changes (filing NDs) |
| Deploy | VM deploy skill — push a locally built Relay to `techgardencode@10.0.60.221` (reuses `vm-e2e` patterns; throwaway/regression host) | — | pending — independent of the PWA; design plan §9 |
