...
affects: "docs/build-plan.md Sequencing table (6J row Status; 6Z row Status + body re-scope); new `Track 8 — Post-2.0 deferrals` section that takes ownership of 6J; docs/prd/07-phasing.md Phase 1 deliverables list + `Phase 1 ships when` line; docs/prd/08-acceptance.md scenario H annotation + `Explicit Phase 1 deferrals` list; docs/phase-1-acceptance-walk.md superseded-by banner; docs/arch/ux-rollout-posture.md §2 + §7 (stale `A–H` framing); .claude/skills/scenario-runner/SKILL.md required reads (stale `A–H` framing)."
...
At time of resolution, 6J (Distribution) is the only Phase 1 implementation row still `pending`. Every other Track 6 row is `done`. The Phase 1 acceptance walk on 2026-05-22 ([`docs/phase-1-acceptance-walk.md`](../phase-1-acceptance-walk.md)) reported `36 pass, 1 n/a, 2 blocked, 0 fail` — the two `blocked` checks are H.1 (`npm install -g @relay/relay` 404s — no npm publish) and H.3 (no `Dockerfile` in tree). Both unblock when 6J ships.
...
8. **Phase 0 surprises §1 (macOS spawn-helper chmod) and §5 (Linux `build-essential`) ride with 6J into Track 8.** They were originally assigned to 6J in [`docs/phase-0-report.md`](../phase-0-report.md); they remain 6J's responsibility under the new track.
