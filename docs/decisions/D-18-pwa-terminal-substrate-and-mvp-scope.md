...
**Surfaces new sub-questions (filed during P2-PWA-tech, 2026-07-02):** the three additive touch points §5 named are now open NDs — [[nd-41-scratch-create-by-path-project-registration]] (scratch / create-by-path), [[nd-42-control-key-claim-release-semantics]] (control-key claim/release), [[nd-43-session-running-idle-status-field]] (running/idle status field) — plus [[nd-44-file-viewer-server-surface]], surfaced by the L4 UI decomposition (FR-13 file viewer). All four are harvested in `docs/arch/pwa/unresolved-dependencies.md` and left for a later server-side pass (client-agnosticism, NFR-5).
