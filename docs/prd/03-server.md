...
- ~~`POST /personas` `GET /personas` `PATCH /personas/:id` `DELETE /personas/:id`~~ — **unmounted at MVP per [D-17](../decisions/D-17-personas-descoped-from-mvp.md)** (personas descoped; these routes return 404 and are re-mounted in Phase 2)
- `POST /sessions` (creates session, spawns a **bare agent** at MVP per [D-17](../decisions/D-17-personas-descoped-from-mvp.md) — body is `{ projectId }`, no `personaName`) `GET /sessions` `DELETE /sessions/:id` (kills agent)
...
> **Deferred to Phase 2 per [D-17](../decisions/D-17-personas-descoped-from-mvp.md) (2026-05-28).** Personas are descoped from the MVP — sessions spawn a bare agent with no persona, so none of the three guarantees below is exercised at MVP. The section is preserved as the Phase 2 contract; the concrete dormant mechanism lives in `arch/persona-application.md`.
...
- ~~`relay persona list`~~ / ~~`relay persona create <name>`~~ — **unwired at MVP per [D-17](../decisions/D-17-personas-descoped-from-mvp.md)** (personas descoped; the `relay persona` command is removed from the dispatcher and the `cli/persona.ts` module stays dormant on disk for the Phase 2 re-enable).
...
- `relay token list` — lists issued tokens, one tab-separated row each: `id`, device label, `createdAt`, and status (`active` for a live token, `revoked <timestamp>` for a revoked one). Revoked tokens are shown by default. Never prints the plaintext or the hash. Reads `~/.relay/tokens.json` directly (read-only; the server process need not be running). This is the recovery path for `relay token revoke <id>` — it is how an operator discovers the `<id>` of a token to kill. *Resolved by [ND-10](../decisions/ND-10-relay-token-list-subcommand-surface-alignment.md) on 2026-05-27.*
...
- `relay doctor` — read-only install diagnostics. Probes, with a remediation line per failure: `~/.relay/` is initialized + writable, `config.yaml` parses, a token store exists, the `claude` binary resolves on `PATH`, Claude credentials are present (via `ANTHROPIC_API_KEY` or the `claude login` OAuth surface per ND-19), and — when a token is available via `RELAY_TOKEN`/`--token` — the server is reachable with a valid token. (The tenant-persona-parse probe is **removed at MVP** with the persona descope per [D-17](../decisions/D-17-personas-descoped-from-mvp.md).) Exits `0` when every probe is OK or WARN, non-zero when any probe FAILs, so it is scriptable. Read-only: it never mutates `~/.relay/` and does not open the SQLite DB. *Resolved by [ND-35](../decisions/ND-35-diagnostics-and-error-ux-deep-dive.md) on 2026-05-27.*
...
> **Persona-driven MCP filtering is deferred to Phase 2 per [D-17](../decisions/D-17-personas-descoped-from-mvp.md).** At MVP the agent spawns bare (empty argv, no `--mcp-config`/`--strict-mcp-config`), so it sees Claude Code's **native** MCP set with no Relay-applied persona filter. The fixed-at-spawn contract below describes the Phase 2 persona behavior.
