...
- `relay doctor` — read-only install diagnostics. Probes, with a remediation line per failure: `~/.relay/` is initialized + writable, `config.yaml` parses, a token store exists, the `claude` binary resolves on `PATH`, Claude credentials are present (via `ANTHROPIC_API_KEY` or the `claude login` OAuth surface per ND-19), tenant personas parse, and — when a token is available via `RELAY_TOKEN`/`--token` — the server is reachable with a valid token. Exits `0` when every probe is OK or WARN, non-zero when any probe FAILs, so it is scriptable. Read-only: it never mutates `~/.relay/` and does not open the SQLite DB. *Resolved by [ND-35](../decisions/ND-35-diagnostics-and-error-ux-deep-dive.md) on 2026-05-27.*
