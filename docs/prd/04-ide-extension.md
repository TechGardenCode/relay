...
- **P0 — Sessions tree view (Activity Bar).** The extension contributes a `viewsContainers` + `views` entry hosting a `vscode.window.createTreeView`: sessions grouped by project, each node exposing attach / release / kill quick actions and a live `running` / `idle` / `killed` status badge (per `01-conceptual-model.md` session lifecycle). The tree refreshes by **REST poll** — the extension never opens a WebSocket, so the claim-arbitration FSM stays inside `relay attach` (the subprocess-of-attach pattern). The poll runs on a fixed cadence only while the view is visible, tears down with the view, and folds a failed poll into an inline error node rather than stopping the loop (the shared poll-lifecycle contract for every REST-poll surface, including the P1 status-bar indicator). The per-node **release** action disposes the `relay attach` terminal the extension spawned for that session — there is no claim in the extension to release directly, so closing the subprocess is what triggers the actual release inside `relay attach`; a session the extension never attached has nothing to release. This is the surface that lets a user "see all my sessions" and switch between them without the command palette or the CLI.
...
*Resolved by [ND-33](../decisions/ND-33-ide-gui-overhaul-deep-dive.md) on 2026-05-26. Tree-view poll cadence + lifecycle resolved by [ND-37](../decisions/ND-37-rest-poll-cadence-and-lifecycle-for-extension-live-surfaces.md) on 2026-05-26.*
