...
**Persona.** _(Deferred to Phase 2 per [D-17](../decisions/D-17-personas-descoped-from-mvp.md) — defined here as the Phase 2 design; **not in force at MVP**, where sessions spawn a bare agent with no persona overlay.)_ A named role context: product, design, dev, test, infra, architect, review by default. Each persona bundles a system prompt overlay (appended to the agent's own prompt), an enabled subset of skills, an enabled subset of MCP servers, and optionally a preferred model. Personas are defined at the tenant level and can be overridden per project — a project-level file fully replaces the tenant-level file of the same name, no per-field merging. Persona definitions are **flat** — there is no inheritance, no `extends`, no override semantics. A user who wants the union of two personas creates a third persona whose skill and MCP subsets are that union. Composition is by selection, not by chaining. The on-disk YAML schema, file paths, and validation rules live in `09-persona-schema.md`; the behavioral guarantees Relay must uphold when applying a persona to a session are specified in `03-server.md` §4.

**Session.** A running agent CLI process bound to a **project** — at MVP the `(project, persona)` pairing is reduced to a project binding, since personas are deferred to Phase 2 per [D-17](../decisions/D-17-personas-descoped-from-mvp.md) (the session row still carries a server-stamped `persona_name` sentinel of `agent`). The session owns the PTY, persists across client disconnections, and is the unit that clients attach to. At MVP the agent spawns bare (no persona overlay); persona binding — and persona changes for an ongoing conversation (a Phase 3 capability via Claude Code's native session-resume) — return with personas in Phase 2+.
...
*Resolved by [D-06](../decisions/D-06-persona-inheritance.md) on 2026-05-14. Persona file schema resolved by [D-09](../decisions/D-09-persona-yaml-schema.md) on 2026-05-15. Project identity (worktrees, multi-root) resolved by [ND-06](../decisions/ND-06-worktree-project-identity.md) and [ND-05](../decisions/ND-05-multi-root-workspace-marker-file-precedence.md) on 2026-05-15. Personas (D-06, D-09) deferred to Phase 2 — the Persona entity and `(project, persona)` binding are not in force at MVP per [D-17](../decisions/D-17-personas-descoped-from-mvp.md) on 2026-05-28.*
