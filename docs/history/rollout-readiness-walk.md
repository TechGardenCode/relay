...
| 10 | Real-LAN proof (`vm-e2e`) | (pre-fix: real binary flapped on the race; `^D` required a follow-up `^C`) | **PASS (2026-05-28)** — laptop server (`0.0.0.0:7777`) + real `claude` 2.1.153 agent; cross-device `relay attach` from the Ubuntu VM (`techgardencode@10.0.60.221`) over the LAN. **ND-40 replay:** the replayed agent frame (1441-byte ring snapshot → 1459 bytes on the wire) was delivered **identically 5/5** — zero drop in the connect→subscribe gap (pre-fix flapped ~20% on loopback, worse over LAN). **ND-38 hang:** a single raw-mode `^D` (sent after the client was connected + in raw mode) returned to the VM shell **3/3**, `rc=0`, each printing the clean `detached … still running` notice — **no hang** (pre-fix it sat until an external `^C`; here the EOF/timeout backstops at 8–15 s were never reached). **D-G3:** session stayed `status: running` across all detaches. **E.1:** VM saw the session via REST (`/sessions` → `items[]`). Tmp trees on both hosts torn down. _Note: real claude 2.1.153 renders **inline** (no `\x1b[?1049h`), so over the LAN the `1049l` screen-restore reset is a confirmed no-op for it (the non-TUI guard case); the alt-screen overlay-restore itself is proven in-process against the alt-screen fixture (#3)._ |

**Net:** ND-38 (Finding F-1) is fixed and locked behind permanent regression tests; ND-40 fixed and quantified; the tui-visual harness now reflects the shipped binary (production ordering + freshness guard) so these cannot silently regress. **The real-LAN `vm-e2e` leg (#10) now passes** — every ledger row is green. ND-39 remains the deferred follow-up (7H).
