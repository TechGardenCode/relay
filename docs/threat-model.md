...
- **Tokens hashed at rest.** `~/.relay/tokens.json` contains salted SHA-256 hashes only (16-byte per-token random salt, `node:crypto`-native, no third-party deps); an attacker who reads the file does not recover usable tokens (D-13, [ND-09](decisions/ND-09-bearer-token-hashing-algorithm.md)). A fast cryptographic hash is sufficient here because tokens carry ≥128 bits of entropy at issue — a slow memory-hard KDF defends low-entropy human credentials against offline brute-force, which is not the threat for high-entropy random secrets.
